...
module fft_butterfly #(parameter width=16)
   ...
   logic signed [width-1:0]    a_re, a_im, b_re_mult, b_im_mult;
   logic signed [width-1:0]    aout_re, aout_im, bout_re, bout_im;
...
module complex_mult #(parameter width=16)
   ...
   logic signed [width-1:0]    term1, term2, term3, term4;
   
   // Inferred DSP Multiplications
   mult #(width) m1 (a_re, b_re, term1);
   mult #(width) m2 (a_im, b_im, term2);
   mult #(width) m3 (a_re, b_im, term3);
   mult #(width) m4 (a_im, b_re, term4);

   assign out_re = term1 - term2;
   assign out_im = term3 + term4;
...
module mult #(parameter width=16)
   ...
   // Lattice Radiant will infer SB_MAC16 for this signed multiplication
   assign result = a * b;
   
   // Q2.14 Fixed Point Truncation
   assign out = result[width+14-1 : 14];
endmodule
