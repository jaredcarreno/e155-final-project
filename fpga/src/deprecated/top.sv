// top.sv - Final 512-point FFT Top Level
module top #(parameter M = 9, WIDTH = 16)(
    ...
    // Generate Divide-by-2 Slow Clock
    ...
    logic [31:0]        core_rd_data; // 32-bit Input to FFT 
    logic [31:0]        core_wd_data; // 32-bit Output from FFT

    // 3. INSTANTIATIONS

    // SPI System (Uses BRAM to save Logic Cells)
    fft_spi_system spi_sys (
        .sck(sck), 
        .sdi(sdi), 
        .sdo(sdo), 
        .reset(reset),
        ...
        .fft_load(core_load),      
        .fft_in_data(core_rd_data),
        .fft_in_addr(core_rd_adr),
        .fft_done(core_done),      
        .fft_out_data(core_wd_data)
    );
