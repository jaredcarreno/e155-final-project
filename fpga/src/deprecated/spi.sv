// spi.sv - BRAM-based SPI Interface for iCE40UP5K
module fft_spi_system (
    ...
    output logic [31:0] fft_in_data,   
    input  logic [8:0]  fft_in_addr,   
    input  logic        fft_done,      
    input  logic [31:0] fft_out_data   
);

    // 1. MEMORY (Inferred Block RAM)
    logic [7:0] rx_mem [0:511];  // Input Buffer
    logic [15:0] tx_mem [0:511]; // Output Buffer

    // Pointers & Registers
    logic [8:0] spi_byte_count;  
    logic [8:0] fft_out_addr;    
    logic [7:0]  shift_reg_in;
    logic [15:0] shift_reg_out;
    logic [3:0]  bit_cnt;        
    logic        sdo_reg;

    typedef enum logic [1:0] {IDLE, RX_TX, SYNC} state_t;
    state_t state;

    // 2. SPI HANDLER (SCK Domain)
        ...
        if (reset) begin
            ...
            case (state)
                IDLE: begin
                    spi_byte_count <= 0;
                    bit_cnt <= 0;
                    shift_reg_out <= tx_mem[0]; // Pre-load first word
                    state <= RX_TX;
                end

                RX_TX: begin
                    // Shift
                    shift_reg_in <= {shift_reg_in[6:0], sdi};
                    ...
                    if (bit_cnt < 15) begin
                        ...
                        // Word Done
                        rx_mem[spi_byte_count] <= {shift_reg_in[6:0], sdi}; // Capture Input
                        
                        if (spi_byte_count < 511) begin
                            shift_reg_out <= tx_mem[spi_byte_count + 1]; // Load Next Output
    ...
    assign sdo = shift_reg_out[15];

    // 3. FFT INPUT (CLK_SLOW Domain)
    logic [7:0] raw_8bit;
    always_ff @(posedge clk_slow) begin
        raw_8bit <= rx_mem[fft_in_addr];
    end
    assign fft_in_data = {8'b0, raw_8bit, 16'b0}; // Zero Pad
    assign fft_load = !fft_done; // Simplified load logic

    // 4. FFT OUTPUT (CLK_SLOW Domain)
    // Capture FFT results into TX RAM
    always_ff @(posedge clk_slow) begin
        if (reset || fft_start) begin
            fft_out_addr <= 0;
        end else if (fft_done) begin
            if (fft_out_addr < 512) begin
                // Store truncated 16-bit {Real[31:24], Imag[15:8]}
                tx_mem[fft_out_addr] <= {fft_out_data[31:24], fft_out_data[15:8]}; 
                fft_out_addr <= fft_out_addr + 1;
            end
        end
    end
