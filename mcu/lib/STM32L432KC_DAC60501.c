...
void configureDAC(){
    // configure SPI
    initSPI(1, 0, 0) // br = 1 (fclk/2), CPOL = 1 (idle hight), CPHA = 0 (clocked on falling edge)
...
void setDAC(float voltage)
{
    ...
    const float GAIN = 2.0f;   // BUFF-GAIN = 1 => gain = 2 (per your setting)

    // Compute DAC code using denominator (2^N - 1)
    float code_f = (voltage * (float)4095) / ((VREF / DIV) * GAIN);
